...
export function Hero({ onBook }: { onBook: () => void }) {
  return (
    <section id="hero" className="relative h-[90vh] min-h-[600px] w-full flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-6 text-center text-white mt-16">
        <motion.span 
          ...
          className="inline-block px-4 py-1.5 rounded-full border border-gold-500/50 bg-black/30 backdrop-blur-md text-gold-400 font-medium tracking-widest text-xs uppercase mb-6"
        >
        ...
        >
          Journey of <span className="text-gold-500 italic">Faith</span> &<br/> Devotion
        </motion.h1>
        <motion.p 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.2 }}
          className="max-w-2xl mx-auto text-lg md:text-xl text-neutral-200 mb-10 font-light"
        >
        ...
        >
          <button 
            onClick={onBook}
            className="group relative inline-flex items-center justify-center px-8 py-4 bg-gold-500 text-emerald-950 font-semibold text-lg overflow-hidden transition-all hover:bg-gold-400 focus:outline-none focus:ring-2 focus:ring-gold-400 focus:ring-offset-2 focus:ring-offset-emerald-900"
          >
      ...
      <div className="absolute -bottom-1 left-0 right-0 w-full overflow-hidden leading-none pointer-events-none">
        <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="block w-full h-[60px] md:h-[120px]" style={{ transform: 'rotate(180deg)' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {valueProps.map((prop, idx) => {
            ...
            return (
              ...
              >
                <div className="flex-shrink-0 w-12 h-12 bg-emerald-900 text-gold-400 rounded-lg flex items-center justify-center">
                  <Icon className="w-6 h-6" strokeWidth={1.5} />
                </div>
                <div>
                  <h3 className="font-semibold text-lg text-emerald-950">{prop.title}</h3>
                  <p className="text-stone-500 text-sm mt-1">{prop.desc}</p>
                </div>
              </motion.div>
            )
          })}
